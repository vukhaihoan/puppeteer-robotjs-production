const M = Math,
    ...
    wait = (ms, s = Date.now(), n = s) => {
        while (n - s < ms) n = Date.now();
    };
...
const moveMouseSmooth = ({ xStart, yStart, xEnd, yEnd, tolerance, confidence, fatigue, speedp }) => {
    let veloX = 0,
        veloY = 0,
        forceX = 0,
        forceY = 0;
    ...
    const applyForce = () => {
        ...
        if (pX != ~~xStart || pY != ~~yStart) robot.moveMouseSmooth(~~xStart, ~~yStart, speedp);
    ...
    if ((~~xEnd != ~~xStart || ~~yEnd != ~~yStart) && !(dist <= tolerance))
        robot.moveMouseSmooth(~~xEnd, ~~yEnd, speedp);
};
...
const typeString = ({ string, tolerance, confidence, fatigue, speed }) => {
    ...
    while (
        commandList.filter((cmd, i) => cmd.typo && (!commandList[i + 1] || !commandList[i + 1].correction)).length >
        tolAmount
    ) {
        let typoList = [],
            r = commandList.filter((cmd, i) => {
